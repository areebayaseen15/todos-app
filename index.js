...
while (condition) {
    ...
    if (operations.selectTask === "update task") {
        ...
        if (add_task.addMore.trim() !== "") {
            let newTodos = todos.filter((val) => val !== update.updatelist);
            todos = [...newTodos, add_task.addMore];
            console.log(chalk.bold.italic.yellow(`Your todoslist is successfully updated!!.Your new updated todolist is`));
    ...
    if (operations.selectTask === "Delete task") {
        ...
        let newTodos = todos.filter((val) => val !== DeleteTask.delete);
        todos = [...newTodos];
        todos.forEach((todo) => console.log(chalk.magentaBright(todo)));
    }
